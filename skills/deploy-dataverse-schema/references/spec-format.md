...
- **`columns[].type`** — one of `String`, `Memo`, `Integer`, `Decimal`, `Money`, `DateOnly`, `Image`, `File`, `Boolean`, `Choice`. `Choice` requires `globalChoiceName` — there is no local-picklist path.
