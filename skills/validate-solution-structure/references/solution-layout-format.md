...
**Shared with `scaffold-solution-structure`** — the same file that `validate-solution-structure` checks live components against is also what `scaffold-solution-structure` reads to know which publisher/solutions to create and which to `pac solution clone` into a local repo. `publisherFriendlyName`/`publisherPrefix` and `layers[].solutionFriendlyName` only matter to the scaffold skill (needed to create something that doesn't exist yet); `validate-solution-structure` ignores them.
  ...
  "layers": [
    {
      ...
      "solutionFriendlyName": "TT Controls",
...
- **`publisherFriendlyName` / `publisherPrefix`** — optional at the top level; only read by `scaffold-solution-structure`, and only used if the publisher doesn't already exist live. `validate-solution-structure` never reads either.
...
- **`layers[].solutionFriendlyName`** — optional; only read by `scaffold-solution-structure`, only used if this layer's solution doesn't exist yet. Defaults to `solutionUniqueName` if omitted.
